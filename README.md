...
| update\_center\_dynamic\_scope\_assignment | Enable dynamic scope assignment for maintenance configurations. | <pre>object({<br/>    enabled     = optional(bool, false)<br/>    name_prefix = optional(string, "mcdc-")<br/>    filter = optional(object({<br/>      locations       = list(string)<br/>      os_types        = optional(list(string), ["Linux", "Windows"])<br/>      resource_groups = optional(list(string))<br/>      resource_types  = optional(list(string))<br/>      tag_filter      = optional(string, "Any")<br/>      tags = optional(list(object({<br/>        key    = string<br/>        values = list(string)<br/>      })), [])<br/>    }))<br/>  })</pre> | `{}` | no |
...
| update\_center\_maintenance\_configurations\_name\_prefix | Prefix for the maintenance configuration names. | `string` | `"mc-"` | no |
