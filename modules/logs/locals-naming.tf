locals {
  ...
  log_analytics_name   = coalesce(var.log_analytics_workspace_custom_name, azurecaf_name.log_analytics_workspace.result)
  storage_default_name = coalesce(var.logs_storage_account_custom_name, azurecaf_name.storage_account.result)
}
