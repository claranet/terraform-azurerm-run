...
resource "azurecaf_name" "storage_account" {
  ...
  prefixes      = local.logs_storage_account_name_prefix == "" ? null : [local.logs_storage_account_name_prefix]
  suffixes      = compact([var.client_name, var.location_short, var.environment, local.name_suffix, "log"])
  use_slug      = var.use_caf_naming
  clean_input   = true
  separator     = "-"
}
